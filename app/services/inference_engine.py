...
class InferenceEngine:
    ...
    def infer(
        ...
        if task_type == "change_detection":
            ...
            rgb = self._color_encode_classes(pred, classes)
    ...
    def infer_batch(
        ...
        for patch_id in patch_ids:
            try:
                path = self.infer(model_id, region_id, patch_id, month)
                results.append(
                    {
                        "patch_id": patch_id,
                        "status": "ok",
    ...
    def _color_encode_classes(
        pred: np.ndarray, classes: List[Dict[str, Any]]
    ) -> np.ndarray:
        ...
        for idx, cls in enumerate(classes):
            ...
            rgb[pred == idx] = InferenceEngine._hex_to_rgb(color)
