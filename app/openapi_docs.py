...
FIELD_DOCS = {
    ...
    "epochs": "训练迭代次数。默认 100，范围 1~1000；当前 few-shot 卷积头服务端最多执行 100 轮，数值越大耗时越长。",
    "class_ids": "自定义训练目标类别 ID。当前 Binary Conv 3x3 few-shot 训练是二分类，每次只能选择 1 个目标类别。",
    "description": "描述文本。用于补充说明模型、任务或配置用途。",
    "annotations": "GeoJSON 标注包。坐标必须是 WGS84，经纬度顺序为 `[lon, lat]`；Polygon 内部作为目标正样本，未标注区域默认忽略。",
