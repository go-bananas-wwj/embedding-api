...
FIELD_DOCS = {
    ...
    "resolved_training_method": "后端实际执行的算法，例如 `sparse_region_model`、`pu_query_retrieval`、`binary_conv3x3` 或 `random_forest`。海淀少样本任务会自动验证并选择前两者。",
    "feature_source": "训练特征来源，例如 `xuannv_embedding` 或 `sentinel2_l2a`。",
    "embedding_version": "训练/推理使用的 embedding 版本。按区域可选 `v1` 或 `v2`。",
    "epochs": "训练迭代次数。默认 100，范围 1~1000。后端按类别分别统计有效 Polygon：某类达到 10 个时训练 Binary Conv 3x3，服务端最多执行 100 轮；某类少于 10 个时使用自动小样本策略，此参数对该类不参与计算。",
    ...
    "annotations": "GeoJSON 标注包。坐标必须是 WGS84，经纬度顺序为 `[lon, lat]`；Polygon 内部是对应类别的正样本，外部是未标注样本而不是直接负样本。后端按 class_id 独立计数和训练：某类少于 10 个 Polygon 时，海淀区自动比较局部区域模型与 PU + Query 并保留验证更稳的方案；大于等于 10 个使用 Binary Conv 3x3。MultiPolygon 的每个独立 Polygon 分别计数。",
