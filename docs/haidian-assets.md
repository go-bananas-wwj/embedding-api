...
`highres_sar` 五类原始 Patch 影像。补充资产还包括哈尔滨道路结果，以及可选的
AEF 2025 外部 embedding。部署归档不重复包含玄女月度 embedding。

海淀施工地的 `202512` 到 `202605` 六个月目录复用同一套静态标注结果，并非
六次独立月度模型推理；这是当前部署约定，调用方不应据此分析月际变化。
