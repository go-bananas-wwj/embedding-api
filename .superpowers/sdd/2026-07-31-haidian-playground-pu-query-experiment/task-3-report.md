...
1. 当前头只有三个 Polygon，训练外观覆盖不足。
2. 生产 PU 训练只从前景相似度最低 30% 的未标注像素中构建背景原型，
   主要学到“容易背景”，缺少屋顶、道路、裸地等高相似度难负样本。
3. 支持 Patch 统计量用于全域标准化，跨 Patch 光谱和场景变化导致分数基线漂移。
4. Query adaptation 是次要放大因素，不是误检的决定性来源。

## 建议

现有面积保护适合做“误检风险控制”对照，但不能直接替换生产结果，因为它
在独立操场上出现 0 召回。下一步画廊应同时展示原始、严格和面积保护结果，
明确这个精度与泛化之间的冲突。
...
最终页面只展示 `patch_000059`、`patch_000060`、`patch_000064`、
