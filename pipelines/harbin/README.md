...
| `raw_harbin.tar` | `/workspace/data/raw/harbin` | mosaic 大图接口使用的 S2/S1/Landsat 原始 TIFF |
| `raw_harbin_scenes.tar` | `/workspace/data/raw/harbin_scenes` | SAM3/embedding 使用的多时相 S2 场景 |
...
python pipelines/harbin/prepare_harbin_api_assets.py \
  --output-root /workspace/data/modelscope-upload/harbin/v1 \
  --dry-run

# 2. 生成 tar 归档（默认输出到 /workspace/data/modelscope-upload/harbin/v1/api_ready）
python pipelines/harbin/prepare_harbin_api_assets.py \
  --output-root /workspace/data/modelscope-upload/harbin/v1
...
modelscope upload --repo-type dataset --token "$MODELSCOPE_TOKEN" \
  ...
  /workspace/data/modelscope-upload/harbin/v1/api_ready \
  harbin/v1/api_ready
```
